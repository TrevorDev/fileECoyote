...
exports.startServer = function (options){
	...
	function *createAccount(){
		if(this.query.masterSecret == options.masterSecret) {
			try {
	...
	function *upload() {
		...
		var acc = yield Account.find({where: {name: this.query.account}})
		if(!acc){
	  ...
	  while (part = yield parts) {
	  	var file = yield File.create({name: part.filename, status: File.STATUS.CREATING})
	...
	function *download() {
		if((yield authenticateApp(this)) || authenticateRequestToken(this)){
			var fileID = this.params.fileID
			var file = yield File.find(fileID)
			...
			if(!file){
				this.jsonResp(400,{message: "Invalid file id"})
				return
			}
			yield options.adaptor.download(this, fileID)
		}else{
	...
	function *authenticateApp(context){
		var acc = yield Account.find({where: {name: context.query.name}})
		if(!acc)
			return false
